...
create table sale_product (quantity int4 not null, product_id uuid DEFAULT uuid_generate_v4() not null, sale_id uuid DEFAULT uuid_generate_v4() not null, primary key (product_id, sale_id));
...
create table tb_sale (id uuid DEFAULT uuid_generate_v4() not null, created_at timestamp not null, note varchar(255), number int4, status varchar(255) not null, updated_at timestamp not null, address_id uuid DEFAULT uuid_generate_v4(), partner_id uuid DEFAULT uuid_generate_v4(), user_id uuid DEFAULT uuid_generate_v4(), primary key (id));
...
alter table if exists tb_sale add constraint UK_95qivs54klj9ityijpbsr79j1 unique (number);
